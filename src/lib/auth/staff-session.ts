...
export type StaffSession = { storeId: string; deviceId: string; role: "staff_terminal"; canManageCatalog: boolean };
...
export async function verifyStaffSession(token?: string | null): Promise<StaffSession | null> { if (!token) return null; try { const { payload } = await jwtVerify(token, sessionSecret()); if (payload.role !== "staff_terminal") return null; return { storeId: String(payload.storeId), deviceId: String(payload.deviceId), role: "staff_terminal", canManageCatalog: payload.canManageCatalog === true }; } catch { return null; } }
