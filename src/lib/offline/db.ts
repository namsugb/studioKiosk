...
export async function flushOutbox(accessToken?: string | null, deviceId?: string | null) {
  const db = await getDb();
  const all = await db.getAll("outbox");
  for (const queued of all) {
    try {
      const response = await fetch("/api/intakes", { method: "POST", headers: { "content-type": "application/json", ...(accessToken ? { authorization: `Bearer ${accessToken}` } : {}), ...(deviceId ? { "x-device-id": deviceId } : {}) }, body: JSON.stringify(queued.submission) });
