...
type Draft = { categoryId: string | null; tierId: string | null; productId: string | null; addonIds: string[]; discountIds: string[]; reviewParticipation: boolean; visaCountryId: string | null; pickupId: string | null; name: string; phone: string; request: string; privacyConsent: boolean };
const emptyDraft: Draft = { categoryId: null, tierId: null, productId: null, addonIds: [], discountIds: [], reviewParticipation: false, visaCountryId: null, pickupId: null, name: "", phone: "", request: "", privacyConsent: false };
...
export function KioskApp({ initialCatalog, accessToken }: { initialCatalog: Catalog; accessToken: string | null }) {
  ...
  useEffect(() => {
    ...
    const sync = () => { setOnline(true); flushOutbox(accessToken).catch(() => undefined); };
    const offline = () => setOnline(false);
    window.addEventListener("online", sync); window.addEventListener("offline", offline); flushOutbox(accessToken).catch(() => undefined);
  ...
  const chooseTier = (id: string) => { const selected = catalog.products.find((item) => item.categoryId === draft.categoryId && item.tierId === id && item.active); if (!selected) return; setDraft((value) => ({ ...value, tierId: id, productId: selected.id, addonIds: [], discountIds: [], reviewParticipation: false, visaCountryId: null, pickupId: null })); window.setTimeout(() => setStep("product"), reduceMotion ? 0 : 120); };
  ...
  const submit = useCallback(async () => {
    ...
    const submission: IntakeSubmission = {
      ...
      addonIds: draft.addonIds, discountIds: draft.discountIds, reviewParticipation: draft.reviewParticipation, visaCountryId: draft.visaCountryId, pickupId: draft.pickupId,
      ...
      selectionSnapshot: { categoryName: category?.name, tierName: tier?.name, productName: product.name, productPrice: product.price, addonIds: draft.addonIds, discountIds: draft.discountIds, reviewParticipation: draft.reviewParticipation, visaCountryId: draft.visaCountryId, visaCountryName: selectedVisaCountry?.name, pickupName: pickup?.name }
    };
    setErrors((current) => ({ ...current, submit: "" }));
    try {
      if (!navigator.onLine) throw new Error("offline");
      const response = await fetch("/api/intakes", { method: "POST", headers: { "content-type": "application/json", ...(accessToken ? { authorization: `Bearer ${accessToken}` } : {}) }, body: JSON.stringify(submission) });
      if (!response.ok) {
    ...
    setStep("complete");
  }, [accessToken, catalog.releaseId, category?.name, draft, pickup?.name, price?.total, product, selectedVisaCountry?.name, tier?.name]);
  ...
  if (step === "idle") return <main className={styles.idle} style={{ "--brand": catalog.studio.primaryColor } as CSSProperties}><div className="brand">{catalog.studio.name}</div><div className={styles.idleMain}><motion.div className={styles.idleVisual} initial={{ scale: .96 }} animate={{ scale: 1 }} transition={transition}><Camera /></motion.div><h1 className="title-xl">촬영 접수를 시작해 주세요</h1><p className={styles.lead}>{catalog.studio.tagline}</p><button className={`btn btn-primary ${styles.idleButton}`} onClick={() => setStep("category")}>촬영 접수하기</button></div><p className="support-copy">{catalog.studio.supportCopy}</p></main>;
  ...
  return <main className="kiosk-wrap"><header className="topbar"><button className="btn btn-ghost" style={{ padding: 0 }} onClick={goBack}><ChevronLeft size={20} /> 이전</button><span className="brand">{catalog.studio.name}</span><span className="caption tertiary">현장 접수</span></header><div className="progress-track"><div className="progress-bar" style={{ width: `${progress}%` }} /></div>{!online && <div className={styles.offline} style={{ margin: "12px 24px 0" }}>인터넷 연결이 없어도 접수할 수 있어요.</div>}<div className={styles.content}><AnimatePresence mode="wait"><motion.section key={step} initial={reduceMotion ? { opacity: 1 } : { opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={reduceMotion ? { opacity: 1 } : { opacity: 0, x: -8 }} transition={transition}>
    {step === "category" && <><h1 className="title-lg">어떤 사진을 촬영하시나요?</h1><p className={styles.lead}>사진 종류를 선택하면 바로 다음 단계로 넘어가요.</p><div className={styles.categoryGrid}>{catalog.categories.filter((item) => item.active).sort((a,b) => a.order-b.order).map((item) => { const Icon = iconMap[item.icon as keyof typeof iconMap] ?? Camera; return <button key={item.id} className={`card card-select ${styles.categoryCard}`} onClick={() => chooseCategory(item.id)}><span className={styles.categoryIcon}><Icon /></span><span><span className={styles.categoryName}>{item.name}</span><span className={styles.categoryDescription}>{item.description}</span></span></button>; })}</div></>}
