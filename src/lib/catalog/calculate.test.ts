...
describe("catalog price rules", () => {
  ...
  it("uses 15~20 minutes for basic and advanced ID and passport products", () => {
    const matchingProducts = defaultCatalog.products.filter((item) => ["id", "passport"].includes(item.categoryId) && (item.tierId === "basic" || item.tierId === "advanced"));
    ...
    const otherBasic = defaultCatalog.products.filter((item) => !["id", "passport", "profile"].includes(item.categoryId) && item.tierId === "basic");
    const otherAdvanced = defaultCatalog.products.filter((item) => !["id", "passport", "job", "profile"].includes(item.categoryId) && item.tierId === "advanced");
  ...
  it("adds print quantity and size details to every ID and passport product", () => {
    const matchingProducts = defaultCatalog.products.filter((item) => ["id", "passport"].includes(item.categoryId));
    expect(matchingProducts).toHaveLength(6);
    for (const item of matchingProducts) {
      expect(item.details).toContain("출력 6매");
      expect(item.details).toContain("규격 3.5 x 4.5 or 3 x 4");
    }
    ...
    expect(advanced.details).toEqual(["3 x 4 8매", "수정본 파일 포함"]);
    expect(advanced.durationLabel).toBe("약 1시간");
    ...
    expect(premium.details).toEqual(["3 x 4 8매", "3.5 x 4.5 6매", "수정본 파일 포함"]);
    ...
    expect(cabinCrew.details).toEqual(["올림머리 헤어스타일링", "치아 보이는 웃는 모습", "3 x 4 8매", "3.5 x 4.5 6매", "수정본 파일 포함"]);
    ...
    expect(cabinCrew.includedAddonIds).toContain("extra-file");
    ...
    expect(premium.tierSubtitle).toBe("헤어·의상 교체 없음");
