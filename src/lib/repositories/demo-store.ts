...
type DemoStore = { id: string; name: string; devices: number };
type DemoDb = { intakes: IntakeRecord[]; catalog: Catalog; releases: Catalog[]; organizations: { id: string; name: string; stores: number; devices: number; storeItems: DemoStore[] }[]; counter: number };
const now = new Date();
const seedIntakes: IntakeRecord[] = [
  ...
  { id: "demo-2", intakeNumber: "A-020", organizationId: "demo-org", storeId: "demo-store", status: "completed", finalTotal: 55000, needsReview: false, createdAt: new Date(now.getTime()-1320000).toISOString(), updatedAt: now.toISOString(), clientSubmissionId: "2af3642e-8c1f-488a-85dc-88aef62a22b7", catalogReleaseId: defaultCatalog.releaseId, categoryId: "job", tierId: "advanced", productId: "job-advanced", addonIds: [], discountIds: [], reviewParticipation: false, visaCountryId: null, pickupId: "same-day", customer: { name: "박서준", phone: "01098765432", request: "" }, privacyConsent: true, expectedTotal: 55000, selectionSnapshot: { productName: "취업사진 고급 보정" } }
];
declare global { var __studioKioskDemoDb: DemoDb | undefined; }
export const demoDb: DemoDb = globalThis.__studioKioskDemoDb ?? { intakes: seedIntakes, catalog: defaultCatalog, releases: [defaultCatalog], organizations: [{ id: "demo-org", name: "나다움 스튜디오", stores: 1, devices: 2, storeItems: [{ id: "demo-store", name: "순천사진관 나다움", devices: 2 }] }], counter: 22 };
